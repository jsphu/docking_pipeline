...
function updateSystemInfo(data) {
  if (data.container_info) {
    document.getElementById("info-hostname").textContent =
      data.container_info.hostname || "unknown";
...
function startPolling() {
  ...
  // Work Polling
  setInterval(updateWork, POLLING_INTERVALS.RESULTS);
  updateWork();
}
...
async function updateGPUStatus() {
  try {
    ...
    document.getElementById("gpu-output").textContent =
      data.output || data.error || "No GPU data available";
  } catch (err) {
...
async function updateResults() {
  try {
    ...
    if (resultFiles.length === 0) {
      ...
      listContainer.innerHTML = resultFiles
        .map(
          (f) => `
                <li>
                    <span>${f}</span>
...
async function updateWork() {
  try {
    ...
    if (workFiles.length === 0) {
      listContainer.innerHTML = '<p class="placeholder">No work files found.</p>';
    } else {
      listContainer.innerHTML = workFiles
        .map(
          (f) => `
                <li>
                    <span>${f}</span>
  ...
  document.getElementById("log-selector").addEventListener("change", (e) => {
    ...
    if (activeLogFile) {
      document.getElementById("log-output").textContent =
        "Loading logs for " + activeLogFile + "...";
      updateLogs();
    } else {
